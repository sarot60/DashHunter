...
func _ready():
	...
	enemy_stats = EnimyStats.get_enemy_stats("Minotaur")
	...
	$HpText/Label.text = str(round(health)) + "/" + str(round(enemy_stats["max_hp"]))
...
func death():
	...
	# Call to GreenVetaDungeon2.gd
	get_tree().call_group("need_boss_ref", "boss_death", true)
	
	create_effect_object.create_drop_coin([10,20])
	create_effect_object.create_item_drop(["Red Gem", "Red Gem", "Rock", "Rock", "Iron Ore"])
