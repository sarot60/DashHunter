...
export (PackedScene) var WaterSpell
...
func death():
	...
	create_effect_object.create_drop_coin([1,10])
	create_effect_object.create_item_drop(["Blue Gem", "Blue Gem", "Blue Gem", "Rock", "Rock"])
...
func water_spell(pos):
	var w = WaterSpell.instance()
	w.position = pos
	#get_parent().add_child(y)
	w.damage = atk
